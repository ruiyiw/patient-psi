export const sessionInstructions = {
    "ccd": "During your talk with the client, please fill in the following fields based on your understanding of the information the client revealed. Please prioritize what you observe during the conversation. ",
    "ccd-situation": "After talking to the client, please fill in the following fields based on the client's description of one specific situation and their reactions to it. If you think the client mentioned multiple situations, please select the situation that is most relevant to the client's mental state or behavior."
}